...
if "%PROMPT_BROKER_DETAILS%"=="1" (
  echo.
  echo ============================================================
  echo  Enter broker details for comparison-only run
  echo ============================================================
  echo.
...
if "%PREVIEW_BEFORE_RUN%"=="1" (
  echo.
  echo ============================================================
  echo  PREFLIGHT CHECK: comparison-only run
  echo ============================================================
  echo.
  ...
  choice /m "Proceed with the comparison-only run"
  if errorlevel 2 exit /b 0
)

echo.
echo ============================================================
echo  RUNNING REAL RESULTS VS BACKTEST ONLY
...
set RUN_CMD=!RUN_CMD! --run-review-now --comparison-only
...
if errorlevel 1 (
  echo.
  echo ERROR: Comparison-only run failed. Review output above.
  if "%KEEP_WINDOW_OPEN%"=="1" pause
  exit /b 1
)

echo.
echo Comparison-only run completed.
if "%KEEP_WINDOW_OPEN%"=="1" pause
exit /b 0
