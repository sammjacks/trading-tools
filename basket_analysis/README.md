...
- `run_basket_analysis_4_filter.cmd` — Step 4: filter optimization- `basket_overlay_research.py` — Step 5: standalone overlay-trade research on top of live basket state
- `run_basket_analysis_5_overlay.cmd` — Step 5 runner for consolidation-based overlay optimisation- `run_combine.cmd` — combine saved curve JSON files across pairs
...
5. Run `run_basket_analysis_5_overlay.cmd` to research a separate consolidation-based overlay trade that keys off the source basket state.
6. Run `run_combine.cmd` if you want to combine saved final-check curves from multiple pairs.
...
- Step 4 now supports Stage 2 basket SL and EOD values in the filter grid.
- Step 5 focuses on a separate overlay trade: many positions open + adverse move + consolidation + standalone SL/TP.
