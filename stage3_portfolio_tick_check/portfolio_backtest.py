"""
portfolio_backtest.py — build the same portfolio twice from the same
fixed-lot backtests: once from bar data and once from tick data.
...
Tick data must be CSV with timestamp,bid,ask in the first three
columns (common Darwinex-style layout). The tool writes separate
outputs for the bar-mapped portfolio and the tick-mapped portfolio
...
Usage:
    python portfolio_backtest.py ^
        ...
        --ticks-dir D:\\Trading\\Ticks ^
        --tick-suffix _GMT+2_US-DST.csv ^
...
def _build_strategy_result(cfg: Dict, trades: List[Dict], base_lot: float,
    ...
    print(f"  {source_label}: {len(trades)} trades  |  net P&L: ${net:,.2f}  |  "
          f"peak: ${peak:,.2f}  |  max DD: ${max_dd:,.2f}  |  "
          f"max open: {max_open}  |  {months:.1f} months")

    return {
        "symbol": cfg["symbol"],
        "display_name": cfg.get("display_name", cfg["symbol"]),
...
def load_strategy(cfg: Dict, months_override: Optional[float] = None,
    ...
    if curve_source == "bars":
        ...
        if not bars:
            raise ValueError(f"{cfg.get('display_name', cfg['symbol'])}: no bars loaded from {cfg['bars_path']}")
        if trades:
    ...
    if curve_source == "ticks":
        ...
        if not ticks:
            raise ValueError(f"{cfg.get('display_name', cfg['symbol'])}: no ticks loaded from {tick_path}")
...
def build_stats_text(strategies: List[Dict], combined: Dict,
    ...
    lines.append("=" * rule_w)
    lines.append("")
    lines.append(f"  {'Strategy file':<{name_w}} {'Scale':>7} {'Trades':>8} {'Max Open':>9} "
    ...
    for s in strategies:
        ...
        sym = _fit_stats_label(s.get("display_name", s["symbol"]), name_w)
        lines.append(
    ...
    lines.append("")
    lines.append(f"  {'Strategy file':<{name_w}} {'Months':>8} {'Lot':>8} {'Open Pos':>9} "
...
def write_portfolio_xlsx(strategies, combined, out_path, title,
    ...
    # ── Table headers (row 7) ────────────────────────────────────────
    headers = [
        "Strategy file", "Trades", "Max Open", "Lot Size", "Months",
...
def main() -> int:
    ...
    def run_source(source_name: str, curve_source: str) -> None:
        ...
        source_out_dir = os.path.join(args.out_dir, source_name)
        ...
        write_portfolio_report(strategies, combined, stats_lines,
                               out_path, f"{args.title} ({source_name})")
        ...
        if not args.no_xlsx:
            ...
            try:
                write_portfolio_xlsx(
                    strategies, combined, xlsx_path, f"{args.title} ({source_name})",
                    args.account_size, args.dd_tolerance, args.backtest_months,
                    None, 0, 0, 0,
                )
    ...
    print(f"\n✓ Stage 3 complete. Compare outputs under: {args.out_dir}")
